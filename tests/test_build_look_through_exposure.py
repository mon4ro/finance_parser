...
def test_direct_stock_exposure_uses_price_symbol_as_holding_key(tmp_path):
    ...
    detail, summary, stats = build_look_through_exposure(master_path, monthly_value_path, holdings_path)
...
def test_fund_exposure_splits_by_holding_percent(tmp_path):
    ...
    unknown_row = detail[detail["HoldingKey"] == "UNKNOWN"].iloc[0]
...
def test_fund_with_no_holdings_snapshot_counted_as_unknown_and_flagged(tmp_path):
    ...
    unknown_row = summary[summary["HoldingKey"] == "UNKNOWN"].iloc[0]
...
def test_month_override_uses_nearest_available_fund_snapshot(tmp_path):
    ...
    detail, summary, stats = build_look_through_exposure(master_path, monthly_value_path, holdings_path, as_of_month="2026-03")
